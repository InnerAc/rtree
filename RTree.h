...
public:
  ...
  typedef bool (*t_resultCallback)(DATATYPE, void*, KINDTYPE);
...
public:
  ...
  void Insert(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], const DATATYPE& a_dataId, KINDTYPE a_kind);
  ...
  /// \param a_kind the kind of search rect
  int Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], t_resultCallback a_resultCallback, void* a_context, KINDTYPE a_kind);
...
protected:
  ...
  bool Search(Node* a_node, Rect* a_rect, int& a_foundCount, t_resultCallback a_resultCallback, void* a_context, KINDTYPE a_kind);
...
RTREE_TEMPLATE
void RTREE_QUAL::Insert(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], const DATATYPE& a_dataId, KINDTYPE a_kind)
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
...
RTREE_TEMPLATE
int RTREE_QUAL::Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], t_resultCallback a_resultCallback, void* a_context, KINDTYPE a_kind)
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
  ...
  int foundCount = 0;
  Search(m_root, &rect, foundCount, a_resultCallback, a_context, a_kind);
...
RTREE_TEMPLATE
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, int& a_foundCount, t_resultCallback a_resultCallback, void* a_context, KINDTYPE a_kind)
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
  ...
  if(a_node->IsInternalNode())
  {
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        if(!Search(a_node->m_branch[index].m_child, a_rect, a_foundCount, a_resultCallback, a_context, a_kind))
        {
  ...
  else
  {
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect)&&a_node->m_branch[index].m_kind == a_kind)
      {
        DATATYPE& id = a_node->m_branch[index].m_data;
        int kind = a_node->m_branch[index].m_kind;
        ++a_foundCount;

        // NOTE: There are different ways to return results.  Here's where to modify
        if(a_resultCallback)
        {
          if(!a_resultCallback(id, a_context ,kind))
          {
